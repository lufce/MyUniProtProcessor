...
sub id_to_full_RecName{
	...
	if($query_id_list_ref eq ""){
		&_function_framework($CODE_id_to_full_RecName);
	}else{
		&_function_framework($CODE_id_to_full_RecName, $query_id_list_ref);
	}
}
...
sub _function_framework{
	...
	while(my $line = <$DB>){
		...
		if($line =~ m/^ID   (.+?) /){
			...
			if($process_code eq $CODE_id_to_full_RecName){ &_process_id_to_full_RecName($DB, $objPB, \@contents) }
			if($process_code eq $CODE_get_all_ID){ push(@contents, $this_id) }
			if($process_code eq $CODE_name_search){ push(@contents, $this_id) }
		}
	}
